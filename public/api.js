window.API = (() => {
  ...
  return {
    ...
    orders: {
      ...
      sumupTerminal:  (id)      => req('POST',   `/api/orders/${id}/sumup-terminal`, {}),
      remove:         (id)      => req('DELETE', `/api/orders/${id}`),
      cancel:         (id)      => req('POST',   `/api/orders/${id}/cancel`),
      history:        (date)    => req('GET',    `/api/orders/history?date=${date}`),
    },
    sumup: {
      pairReader:    (code, name) => req('POST',   '/api/sumup/reader/pair', { pairing_code: code, name }),
      listReaders:   ()           => req('GET',    '/api/sumup/reader/list'),
      readerStatus:  ()           => req('GET',    '/api/sumup/reader/status'),
      unpairReader:  ()           => req('DELETE', '/api/sumup/reader'),
      terminateTerminal: ()       => req('POST',   '/api/sumup/terminal-terminate', {}),
    },
